...
taumel	- thanks for the cool modifications to the grid effects
and the bloom effect on the gfx sets
...
Mark Incitti + all of the above for hours of childhood fun. 

mmatyas and trap000d on Github for previous work compiling on Raspberry Pi. (See https://github.com/mmatyas/GridWars.)

Geometry Wars devs for making a remarkably similar game, first, which I never got to play.

And Portmaster developers for making this port possible.
...
ESC/left shoulder button to bring up options menu -> Configure to your setup
...
------------------------------------------------------------------------------
|  | Original | Retro Handhelds
|--|----------|-----------------
|Screen | 1024X768 | native (auto-applied)
| Playfield | 1024X768 | Similar, but matching native aspect ratio
| Scroll | Off | Off
| Inertia |	50% | 50%
| GFX | High | High
| Stars	| 400 | 400
...
Left shoulder/trigger/start: Menu/ESC

Right shoulder/trigger: Bomb/Select

Dual Analog - Left joystick move, right joystick fire.

Joypad - Left joystick move, ABXY buttons fire.

"Keyboard" - D-pad move, ABXY buttons fire.

**Keyboard/Mouse**

Mouse - 360 degree move, LMB fire, RMB bomb  (ESC key for config) NB: For retro handheld port, mouse is disabled in menus because I'm a lazy arse. - Dr. C
...
Config:
---------------------------------------------------------------------------
Here's an explanation of the various things in the config.txt file. Be careful editing these values (very dependent on [<Label>] followed by value on next line) Most of these are configurable through the various options menus.

*these can only be changed manually.

**Doctor's note:** Apparently some of these are order-dependent. For example, if some joystick entries are missing, it will screw things up. If you mess it up, delete Config.txt and the launch script will generate a same new one.

The game may not launch if the resolution is set incorrectly.
```
[Windowed]
...
[Show Stars]
0-1000 (number of stars in background)
